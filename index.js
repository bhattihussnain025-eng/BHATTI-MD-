...
// Owner numbers
const ownerNumber = ["923181742221"];
...
function loadLocalFiles() {
  ...
  // Check for bhatti.html
  if (fs.existsSync('./lib/arslan.html')) {
    console.log("✅ bhatti.html found in lib folder");
  } else {
    console.log("⚠️ bhatti.html not found in lib folder");
  }
...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("BHATTI-MD~")) {
    console.log("❌ Invalid BHATTI-MD session format");
    process.exit(1);
  }
  ...
  console.log("♻️ BHATTI-MD session restored successfully");
}
...
// Serve HTML page
app.get('/', (req, res) => {
  const htmlPath = path.join(__dirname, 'lib', 'BHATTI.html');
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          <p>Owner: ${config.OWNER_NAME || 'bhatti'}</p>
        </div>
      </body>
      </html>
    `);
...
app.get('/lib/bhatti.html', (req, res) => {
  res.redirect('/');
});
...
async function getProfilePicture(sock, jid) {
  try {
    ...
    return 'https://kommodo.ai/i/LyNZMOKK852A7m8h70UG'; // Default image
  }
}
...
async function connectToWA() {
  ...
  console.log(`🤖 BHATTI-MD Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ BHATTI-MD Bot connected to WhatsApp!");
      ...
      const aliveMsg = `*╭──────────────●●►*\n> *BHATTI-MD CONNECTED SUCCESSFULLY*\n\n> *Type ${prefix}menu to view commands*  \n\n*╭⊱✫ BHATTI MD ✫⊱╮*\n*│✫📂 Bot Name: ${botConfig.BOT_NAME}*\n*│✫🛡️ Owner: ${config.OWNER_NAME}*\n*│✫♻️ Prefix: ${prefix}*\n*│✫🌍 Mode: ${config.MODE}*\n*│✫⏰ Uptime: ${runtime(process.uptime())}*\n*╰──────────────●●►*\n\n> Enjoy Using BHATTI MD`;
      
      // Image URL for connection message
      const imageUrl = 'https://kommodo.ai/i/LyNZMOKK852A7m8h70UG
      
      try {
        // Send to owner with image
        sock.sendMessage(ownerNumber[0] + '923181742221@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
